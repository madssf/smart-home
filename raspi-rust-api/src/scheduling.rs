...
use chrono::{Datelike, NaiveDateTime, Weekday, NaiveTime, Duration};
...
impl ScheduleData {
    fn from_json(json: &ScheduleDataJson) -> Self {
        ScheduleData {
            ...
            days: json.days.iter()
                .map(|day| {
                    Weekday::from_str(day).expect("Failed to parse weekday")
                })
                .collect::<Vec<Weekday>>(),
            windows: json.hours.iter().map(|x| {
...
fn read_schedules_from_file() -> String {
    let schedules = fs::read_to_string("./schedule.json").expect("Missing schedule.json");
    schedules
}

fn parse_schedules_string_to_json(schedules: &str) -> Vec<ScheduleDataJson> {
    let schedules: Vec<ScheduleDataJson> = serde_json::from_str(schedules)
        .expect("Invalid json format");
    schedules
}

fn parse_schedules_json_to_domain(schedules: &Vec<ScheduleDataJson>) -> Vec<ScheduleData> {
    let schedules = schedules
        .iter()
        .map(|json| ScheduleData::from_json(json))
...
fn matching_schedule(schedule_data: &ScheduleData, price_level: &PriceLevel, time: &NaiveDateTime) -> bool {
    return price_level == &schedule_data.level
        && schedule_data.days.contains(&time.weekday())
        && schedule_data.windows.iter()
            .map(|window| {
                window.0 < time.time() && window.0 + window.1 > time.time()
            })
...
#[cfg(test)]
mod tests {
    use chrono::{Weekday, NaiveTime, Duration, NaiveDateTime};

    use crate::prices::PriceLevel;

    use super::{parse_schedules_json_to_domain, ScheduleDataJson, HoursJson, ScheduleData, matching_schedule};
    ...
    fn should_only_return_matching_schedule() {
        ...
        assert!(matching_schedule(&schedule_data, &PriceLevel::CHEAP, &NaiveDateTime::from_timestamp(1663592399, 0)));
        
        // Wrong level
        assert!(!matching_schedule(&schedule_data, &PriceLevel::NORMAL, &NaiveDateTime::from_timestamp(1663592399, 0)));

        // Wrong day
        assert!(!matching_schedule(&schedule_data, &PriceLevel::CHEAP, &NaiveDateTime::from_timestamp(1663765199, 0)));

        // Wrong time
        assert!(!matching_schedule(&schedule_data, &PriceLevel::CHEAP, &NaiveDateTime::from_timestamp(1663678800, 0)));
